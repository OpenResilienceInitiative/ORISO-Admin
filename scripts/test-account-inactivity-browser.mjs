...
const scenario = async (name, check) => {
    ...
    try {
        ...
        await page.goto(`${server.resolvedUrls.local[0]}src/test/account-inactivity.html`);
        await useNativeFocus(context, page);
        await page.bringToFront();
        await page.getByRole('button', { name: 'Use account A' }).click({ timeout: 10000 });
        assert.equal(requests.length, 0, 'Signing in alone must not report client activity');
        await check({
            page,
            context,
            requests,
...
const expectReport = (page, gesture) => Promise.all([page.waitForResponse(endpoint, { timeout: 3000 }), gesture()]);
try {
    await scenario('real pointer gesture reports authenticated self with no body', async ({ page, requests }) => {
        await expectReport(page, () => page.getByRole('textbox', { name: 'Write a message' }).click());
        assert.equal(requests.length, 1);
        assert.equal(requests[0].method, 'POST');
        assert.equal(requests[0].body, null, 'The server supplies its own timestamp');
        assert.match(requests[0].headers.authorization, /^Bearer header\./);
        assert.ok(requests[0].headers['x-csrf-token']);
    });
    await scenario('synthetic DOM input does not count as personal activity', async ({ page, requests }) => {
        await page.evaluate(() => {
    ...
    await scenario('real foreground keyboard input reports activity', async ({ page, requests }) => {
    ...
    await scenario(
        ...
        async ({ page, context, requests }) => {
            ...
            await foreground.setContent('<input aria-label="Other application" />');
            await foreground.getByRole('textbox', { name: 'Other application' }).click();
            ...
            const state = JSON.parse(await foreground.locator('body').getAttribute('data-gesture'));
            assert.equal(state.trusted, true, 'The test must use a real browser event');
            assert.equal(state.focused, true, 'The genuine key event belongs to the other tab');
            assert.equal(await page.evaluate(() => document.hasFocus()), false);
            await page.waitForTimeout(100);
            assert.equal(requests.length, 0);
    ...
    await scenario(
        ...
        async ({ page, requests }) => {
            ...
            await page.getByRole('button', { name: 'Refresh account A token' }).click();
    ...
    await scenario('account switches reset the successful-report throttle', async ({ page, requests }) => {
    ...
    await scenario('logout stops reporting and unmount removes listeners', async ({ page, requests }) => {
        await expectReport(page, () => page.getByRole('textbox').click());
        await page.getByRole('button', { name: 'Log out' }).click();
        await page.getByRole('textbox').click();
        await page.keyboard.press('a');
        assert.equal(requests.length, 1);
        await page.getByRole('button', { name: 'Unmount tracking' }).click();
        await page.getByRole('button', { name: 'Use account B' }).click();
        await page.getByRole('textbox', { name: 'Tracking removed' }).click();
        await page.keyboard.press('a');
        await page.waitForTimeout(100);
        assert.equal(requests.length, 1);
    });
    ...
    if (process.env.VERIFY_REAL_INTERVAL === '1') {
        await scenario('60 seconds alone never reports; the next genuine gesture does', async ({ page, requests }) => {
            await expectReport(page, () => page.getByRole('textbox').click());
            await page.waitForTimeout(60_100);
            assert.equal(requests.length, 1, 'Elapsed time cannot extend personal activity');
            await expectReport(page, () => page.keyboard.press('a'));
            assert.equal(requests.length, 2);
        });
    }
