...
-   `dev` is the integration branch; `main` is release. Never commit directly to either.
...
-   Force-push is prohibited on `dev` and `main`. On your own feature branch it is allowed only before review starts; after review starts, add normal commits.
