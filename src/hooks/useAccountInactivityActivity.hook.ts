...
export const useAccountInactivityActivity = () => {
    useEffect(() => {
        ...
        const reportGesture = (event: Event) => {
            ...
            if (
                account.pending ||
                (account.lastSuccess !== null && Date.now() - account.lastSuccess < REPORT_INTERVAL_MS)
            )
                return;
            ...
            reportAccountInactivityActivity(token, controller.signal)
                .then((success) => {
                    if (success && !disposed && account === currentAccount) currentAccount.lastSuccess = Date.now();
                })
                .catch(() => undefined)
                .finally(() => {
