...
import { recordLoginFailure, resetLoginFailureTrackerForTests } from './loginFailureTracker';
...
describe('loginFailureTracker', () => {
    ...
    it('counts one failure with the dashboard-contract names and attributes', () => {
        recordLoginFailure({ outcome: 'credentials', transport: 'bad_request', stage: 'otp' });
        expect(mocks.getMeter).toHaveBeenCalledWith('login-tracker');
        expect(mocks.createCounter).toHaveBeenCalledWith(
            'login_failure',
            expect.objectContaining({ description: expect.any(String) }),
        );
        expect(mocks.add).toHaveBeenCalledWith(1, { outcome: 'credentials', transport: 'bad_request', stage: 'otp' });
    });

    it('creates the counter once and reuses it', () => {
        recordLoginFailure({ outcome: 'unavailable', transport: 'network', stage: 'password' });
        recordLoginFailure({ outcome: 'unavailable', transport: 'network', stage: 'password' });
    ...
    it('never carries anything but the three health attributes', () => {
        ...
        expect(Object.keys(mocks.add.mock.calls[0][1]).sort()).toEqual(['outcome', 'stage', 'transport']);
    });
    ...
    it('swallows meter failures instead of breaking the login screen', () => {
        ...
        expect(() =>
            recordLoginFailure({ outcome: 'credentials', transport: 'unauthorized', stage: 'password' }),
        ).not.toThrow();
    });
});
