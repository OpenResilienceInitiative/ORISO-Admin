...
/**
 * Ported from ORISO-Frontend/src/components/twoFactorAuth/twoFactorSetupFlow.test.ts —
 * the assertions are the shared flow contract between the app and admin layers.
 */
describe('twoFactorSetupFlow', () => {
    it('routes the decision step into the selected app or email flow', () => {
        expect(getNextSetupStep('decision', TwoFactorType.App)).toBe('app-install');
        expect(getNextSetupStep('decision', TwoFactorType.Email)).toBe('email-select');
    });
    ...
    it('keeps app flow ordering stable', () => {
        ...
        expect(getNextSetupStep('app-connect', TwoFactorType.App)).toBe('app-verify');
    ...
    it('keeps email flow ordering stable', () => {
        ...
        expect(getNextSetupStep('email-select', TwoFactorType.Email)).toBe('email-connect');
        expect(getPreviousSetupStep('email-connect')).toBe('email-select');
    });

    it('derives method and index from the active setup step', () => {
        expect(getStepMethod('email-connect')).toBe(TwoFactorType.Email);
        expect(getStepMethod('app-verify')).toBe(TwoFactorType.App);
