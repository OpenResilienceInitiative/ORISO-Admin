/**
 * Admin icon catalog (Design System/Icons).
 *
 * Renders `iconCatalog.generated.json`, which is produced by
 * `scripts/generateIconCatalog.mjs` and committed to the repo. Regenerate with
 * `npm run generate:icon-catalog` after adding/renaming/removing icon assets
 * or changing icon references in app source (the prestorybook /
 * prebuild-storybook hooks also regenerate it automatically), then commit the
 * JSON diff.
 */
...
const meta = {
    ...
    parameters: {
        ...
        docs: {
            description: {
                component:
                    'Searchable, machine-readable catalog generated from ORISO Admin assets. App-used excludes tests and Storybook-only references. Data source: `src/components/IconCatalog/iconCatalog.generated.json`, produced by `npm run generate:icon-catalog` (auto-run by the prestorybook / prebuild-storybook hooks). Re-run and commit the JSON diff after adding, renaming or removing icon assets.',
            },
        },
    },
