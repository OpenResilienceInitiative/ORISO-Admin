 ...
 * (ORISO-UserService#914). **This repository owns none of that markup.** The backend renders it
 * once — `InviteFrameMailRenderer` with the ORISO e-mail frame template
 * `emails/<tone>/einladung-freitext.*`, generated by the ORISO-Frontend e-mail kit — and the Admin
 * only ever displays the result: here from checked-in fixtures, in the app from the live preview
 * endpoint.
 *
 ...
 * (the platform name next to the logo slot).
 *
 ...
 * next to the wordmark, and `theming.primaryColor` (`#f8e71c`) drives the accent bar above the
 * subject. Note the contrast guard: the button label is always white, and yellow fails 4.5:1
 * against white, so the button falls back to the ORISO primary (`#a5000a`) instead of turning
 * yellow.
 *
...
/**
 * **Call-to-action present** (the invite default) versus **absent**. `InviteFrameMailRenderer`
